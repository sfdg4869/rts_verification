"""
RTS: run Target workload, then verify ORA_SQL_* rows in the Repo DB.
"""
...
def _pg_apm_db_info_sql_variants(sql: str, schema_name: Optional[str]) -> List[str]:
    variants: List[str] = [sql]
    if "public.apm_db_info" not in sql:
        return variants

    schema = str(schema_name or "").strip()
    candidates: List[str] = []
    if schema and schema.lower() != "public":
        candidates.append(f"{schema}.apm_db_info")
    candidates.append("apm_db_info")

    for ref in candidates:
        candidate_sql = sql.replace("public.apm_db_info", ref)
        if candidate_sql not in variants:
            variants.append(candidate_sql)
    return variants


def _query_rows_with_pg_apm_db_info_fallback(
    cursor,
    sql: str,
    ...
    variants = _pg_apm_db_info_sql_variants(sql, schema_name)
    rows: List[Dict[str, Any]] = []
    for i in range(max_attempts):
        for candidate_sql in variants:
...
def run_new_repo_check(
    ...
    target_cfg = {
        ...
        "port": int(target_config.get("port") or 1521),
        "user": target_user,
    ...
    target_svc = OracleService(target_cfg)

    conn = target_svc.connect()
    if not conn:
        _progress(1, 6, "target_connect", "fail")
        return {"overall_status": "error", "error": f"Target DB ({target_user}) 연결 실패", "data": result_data}
    ...
    try:
        ...
        try:
            ...
            pass
        target_svc.release_connection(conn)
    ...
    try:
        ...
        if repo_engine == "postgresql":
            ...
            result_data["repo_stat"] = _query_rows_with_pg_apm_db_info_fallback(
                repo_cur, q_stat, schema_name, max_attempts=4, wait_seconds=8
            )
        else:
            result_data["repo_elapse"] = _query_rows_with_retry(repo_cur, q_elapse, max_attempts=4, wait_seconds=8)
            result_data["repo_stat"] = _query_rows_with_retry(repo_cur, q_stat, max_attempts=4, wait_seconds=8)
...
def run_step5_repo_only(
    ...
    try:
        ...
        if repo_engine == "postgresql":
            ...
            result_data["repo_stat"] = _query_rows_with_pg_apm_db_info_fallback(
                repo_cur, q_stat, schema_name, max_attempts=4, wait_seconds=10
            )
        else:
            result_data["repo_elapse"] = _query_rows_with_retry(repo_cur, q_elapse, max_attempts=4, wait_seconds=8)
    ...
    if pol_repo_config_id and overall_status != "error":
        ...
        try:
            ...
            if pol_engine == "postgresql":
                result_data["repo_elapse_pol"] = _query_rows_with_pg_apm_db_info_fallback(
                    pol_cur, pq_e, pol_schema, max_attempts=4, wait_seconds=8
                )
                result_data["repo_stat_pol"] = _query_rows_with_pg_apm_db_info_fallback(
                    pol_cur, pq_s, pol_schema, max_attempts=4, wait_seconds=10
                )
            else:
    ...
    if sys_password and target_config:
        try:
            ...
            scur.close()
            sys_conn.close()
            clean_msg = ("SYS shared pool purge + drop procedures success: " if not drop_failed else "SYS cleanup partial: ") + ", ".join(s6_results)
        except Exception as e:
            clean_msg = f"SYS cleanup failed: {e}"
    result_data["cleanup_info"] = clean_msg
