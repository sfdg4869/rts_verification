...
def _rollback_cursor_connection(cursor) -> None:
    ...
    for i in range(max_attempts):
        for candidate_sql in variants:
            try:
                ...
                last_error = exc
                _rollback_cursor_connection(cursor)
                continue
        if i < max_attempts - 1:
            time.sleep(wait_seconds)

    if last_error is not None and not had_success:
        raise last_error
    return rows
        ...
        try:
            ...
            if pol_engine in ("postgresql", "postgres"):
                result_data["repo_elapse_pol"] = _query_rows_with_pg_apm_db_info_fallback(
                    pol_cur, pol_q_elapse, pol_schema, max_attempts=4, wait_seconds=8
                )
                result_data["repo_stat_pol"] = _query_rows_with_pg_apm_db_info_fallback(
                    pol_cur, pol_q_stat, pol_schema, max_attempts=20, wait_seconds=30
                )
            else:
